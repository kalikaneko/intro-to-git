...
usuario=`whoami`
	dir=`zenity --filename=/home/$usuario/Guarango\ Radio/comerciales/ --file-selection --directory --title="Seleccione una Carpeta de comerciales para reproducir aleatoriamente"`
...
echo "$dir""|""$diasemite""|""$pautaini""|""$pautafin" > ~/.guarangoradio/data/tmp/comercialg3    
else
exit
fi
exit 0
